...
function* createNoise() {
	...
	for (;;) {
		...
		latestOutput =
			latestOutput.length < 2 ? latestOutput + nextOutput : latestOutput.slice(1) + nextOutput;
		yield nextOutput;
	}
}
...
const command: Command = {
	...
	run: async (message, _context) => {
		...
		if (Number.isNaN(targetLength)) {
			await message.channel.send('No valid length specified.');
			return;
		}
