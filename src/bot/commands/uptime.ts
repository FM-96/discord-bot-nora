...
const command: Command = {
	...
	run: async (message, _context) => {
		...
		await message.channel.send(
			`Time since program start: **${timeSinceStartup}**\nTime since last login: **${timeSinceLogin}**`,
		);
	},
};
...
function formatDuration(duration: number) {
	...
	const durationDisplayMinutes =
		Math.floor(durationTotalSeconds / 60) -
		durationDisplayHours * 60 -
		durationDisplayDays * 60 * 24;
	const durationDisplaySeconds =
		durationTotalSeconds -
		durationDisplayMinutes * 60 -
		durationDisplayHours * 60 * 60 -
		durationDisplayDays * 60 * 60 * 24;
